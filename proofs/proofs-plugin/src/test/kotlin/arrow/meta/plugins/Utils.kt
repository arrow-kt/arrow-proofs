...
fun ConfigSyntax.newMetaDependencies(): List<Config> {
  ...
  val proofsCompilerPlugin =
    CompilerPlugin("Arrow Meta Proofs", listOf(
      Dependency("arrow-proofs-plugin:$currentVersion")
    ))
  return metaDependencies + addCompilerPlugins(proofsCompilerPlugin) + addDependencies(Dependency("arrow-meta-prelude:$currentVersion"))
}
