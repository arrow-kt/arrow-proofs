...
class PluginAnnotationsProvider(testServices: TestServices) :
  ...
  companion object {
    private const val ANNOTATIONS_JAR_DIR = "../inject-annotations/build/libs/"
    private val ANNOTATIONS_JAR_FILTER = FilenameFilter { _, name ->
      name.startsWith("inject-annotations") && name.endsWith(".jar")
    }
  }
  ...
  private val failMessage = {
    "Jar with annotations does not exist. Please run :inject-annotations:jar"
  }
}
